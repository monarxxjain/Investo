...
import { signMessage,initWallet } from "@/utils/etherInterface";
...
const LoginForm = ({ setIsSnackbarOpen, existingEmail, userData, setUserData, view, setView }) => {
  ...
  const connectWallet = async () => {
    try {
      ...
      if(walletData.connected) {
        setUserData((prev) => ({
          ...prev, modelData: {
            ...prev.modelData, wolleteAddr: walletData.account.address
          }
        }))
        if(userData.role == "INVESTOR"){
          await loginInvestor(walletData.account.address)
        }
  ...
  const loginInvestor = async (wolleteAddr) => {
    try {
